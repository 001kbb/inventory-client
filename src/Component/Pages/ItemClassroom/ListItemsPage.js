...
export default function ListItemsPage() {
    ...
    useEffect(() => {
        if (connection) {
          connection
            .start()
            .then(() => {
              connection.on("SendReload", (message) => {
                fetchItems()
                fetchStats()
              })
            })
    ...
    const fetchStats = useCallback(async () => {
        try {
            setStatsLoading(true)
             const data = await request('/api/classroom/stats/'+id, 'GET', null,  {'Authorization': 'Bearer ' + token})
             setStats(data)
    ...
    return (
        ...
        />
            ...
            {(loading && !isSearching) && <div>{t('Loading...')}</div>}
