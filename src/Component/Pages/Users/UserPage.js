  ...
  const {t} = useTranslation()
    ...
    const list = reports.map(x => {
        return <tr>
            ...
            <td><button onClick={() => window.location.replace(x.xlsUrl)}>{t('Download Excel')}</button></td>
        </tr>
    })
        ...
        <>
          ...
          {!loading && (
            <div className="user-page">
              ...
              <button className="user-page-button-right1" onClick={async () => {
                  try {
              ...
              {logs !== null && <div><button className="user-page-button-right2" onClick={() => window.location.replace(logs.url)}>{t('Open logs')}</button></div>}
              </div>
